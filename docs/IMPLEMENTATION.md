...
### 最終統合検証 — 全3ステージクリアと2P操作

`d93966e` のActionsビルドを同じDuckStationで再読み込み。通常のエミュレーション入力経路で全3ステージをクリアし、ARCADE CHAMPIONを確認。全通し3,777フレームはすべて1 VBlankだった。

![全3ステージクリア](screenshots/015-arcade-champion.png)

道着モデル2体の2P戦では、2番パッドの選択・移動・パンチ・ポーズを検証。しゃがみ中は上段パンチを回避し、立つとHPが減少した。L2のバックステップで後退し、リング端を越えるとリングアウトする。ポーズ込み3,197フレームはすべて1 VBlank。

![しゃがみで上段回避](screenshots/016-versus-crouch.png)

![2番パッドのポーズ](screenshots/017-versus-pause.png)

![バックステップ後のリングアウト](screenshots/018-versus-backstep-result.png)

[最終計測](frame-audit-final.txt)：合計6,974フレーム、遅延0。元の入力プリセットを復元し、Pad1/Pad2の一致を確認。検証用TTY・ファイルログを解除。物理PS1／物理ゲームパッドは未試験。
